# GLaDOS / Railgun 自动签到
基于 Cloudflare Workers 的自动签到：支持多站点（GLaDOS、Railgun 等）、多账号、网页查看状态、手动触发与定时任务；可选 Telegram 通知推送。

![预览](glados.jpg)

- GLaDOS 签到页：https://glados.cloud/console/checkin
- Railgun 签到页：https://railgun.info/console/checkin
...
- 多站点（内置 GLaDOS + Railgun，同一 Cookie 自动签到所有站点）
- 多账号（用 `&` 分隔多个 Cookie）
- Token 自动检测（从签到页面自动提取，无需手动配置）
- Telegram 通知（可选）
- 积分兑换延长时长（可选）
...
```
GLADOS_COOKIE=你的Cookie
```

内置站点（glados.cloud + railgun.info）会自动使用同一个 Cookie 签到。多账号用 `&` 分隔：`cookie1&cookie2`

#### 可选变量

```
# 覆盖内置站点列表（JSON 数组），一般无需设置
SITES=[{"name":"xxx","url":"https://xxx.com"}]

# 按站点分配不同 Cookie（优先级高于 GLADOS_COOKIE）
SITES_COOKIES={"glados":"cookie1","railgun":"cookie2"}

GLADOS_CHECKIN_TOKEN=xxx  # 可选；通常无需设置，worker 会自动从签到页面提取 token
                            # 内置候选：railgun.info / glados.cloud / glados.one 等
                            # 仅当自动检测失败且站点 token 特殊时才需手动指定
...
- `GLADOS_COOKIE` 默认应用到所有内置站点；如需按站点使用不同 Cookie，设置 `SITES_COOKIES`

Cookie 请自行抓包获取，格式示例：
`koa:sess=...; koa:sess.sig=...`

#### 如何获取 Cookie
这两种方法最终目的都是拿到 `Cookie` 请求头；区别在于复制位置不同。适用于 GLaDOS 和 Railgun 等同类站点。

**方法一：从 Cookies 面板取值并手动拼接（推荐）**
1. 打开浏览器登录目标站点（如 `https://glados.cloud` 或 `https://railgun.info`）
2. 按 `F12` 打开开发者工具
3. Chrome：`Application` → `Cookies`；Firefox：`存储(Storage)` → `Cookies`
4. 选择对应域名，找到并复制 `koa:sess` 与 `koa:sess.sig` 两个条目的值
5. 按下面格式拼接为一行（注意分号与空格）：`koa:sess=...; koa:sess.sig=...`

**方法二：Network 直接复制请求头（最省事）**
1. 打开目标站点的签到页（如 `https://glados.cloud/console/checkin` 或 `https://railgun.info/console/checkin`）
2. `F12` → `Network/网络`，刷新页面（`Ctrl + R`）
3. 点开任意一个发往该域名的请求（例如 `api/user/status` / `api/user/checkin`）
4. 在 `Request Headers/请求标头` 中找到 `Cookie`，复制其完整一整段

将复制到的内容填入环境变量 `GLADOS_COOKIE`；多账号用 `&` 连接：`cookie1&cookie2`  
